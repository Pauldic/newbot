...
from bot.forms import UserForm, EngagementRuleForm, SetupForm, NewsSourceFormSet
from bot.models import Message, Coin, Result, Client, Defaulter, Group, Member, Leech, IDefaulter, EngagementRule, \
    Setup, NewsSource, News
from django_telegrambot.apps import DjangoTelegramBot
from django.db import connection

from bot.tasks import news_feed_check
from bot.utils import TinClient, removeUser, sendMsg, addUser, TeleClient
...
@login_required
def member_demote(request, member_id, action="demote"):
    return member_promote(request, member_id, action=action)


@login_required
def member_promote(request, member_id, action="promote"):
    member = get_object_or_404(Member, member_id=member_id)
    if action == "promote":
        if member.type == Member.EDITOR:
            messages.warning(request, "Member is already an Editor")
        else:
            member.type = Member.EDITOR
            member.save()
            messages.success(request, "Member successfully Promoted to Editor")
    else:
        if member.type == Member.MEMBER:
            messages.warning(request, "No action!!! Member is not an Editor")
        else:
            member.type = Member.MEMBER
            member.save()
            messages.success(request, "Member successfully Promoted to Member")

    return render(request, 'bot/member-details.html', context={'context': Member.objects.filter(), 'type': app_type,})
...
def member(request, member_id=None, group_id=None):
    ...
    if request.method == "POST":
        if request.POST.get('reward', None) and context.first():
            context = context.first()
            context.reward += Decimal(request.POST.get('reward'))
            context.save()
            return JsonResponse({"type": "Success", "msg": "Rewarded Successfully"})
...
@login_required
def sources(request, id=None):
    sources = NewsSource.objects.filter()
    if request.method == "POST":
        formset = NewsSourceFormSet(request.POST, queryset=sources, prefix='src')
        if formset.is_valid():
            formset.save()
            messages.warning(request, "News Sources Updated Successfully", "success")
            return redirect(reverse("core:sources"))
        else:
            messages.error(request, "News Sources Updated Error!!!", "error")
            log.warning(formset.errors)
    else:
        formset = NewsSourceFormSet(queryset=sources, prefix='src')

    return render(request, 'bot/source-form.html', context={'formset': formset, 'sources': sources.filter(enabled=True),})


@login_required
def bot_setup(request, group_id=None):
    # log.warning("Group {}".format(group_id))
    # if group_id:
    #     group = Group.objects.filter(id=group_id).first()
    # else:
    #     group = Group.objects.filter(enabled=True).first()

    if request.method == "POST":
        form = SetupForm(request.POST, instance=Setup.objects.filter(enabled=True).first())
        if form.is_valid():
            form.save()
            messages.warning(request, "Setup Updated Successfully")
            return redirect(reverse("core:bot-setup"))
        else:
            log.warning(form.errors)
    else:
        form = SetupForm(instance=Setup.objects.filter(enabled=True).first(), )

    # return render(request, 'bot/setup-form.html', context={'group': group, 'form': form, 'type': app_type,})
    return render(request, 'bot/setup-form.html', context={'form': form, 'type': app_type,})


@login_required
def bot_setup_detail(request):

    return render(request, 'bot/setup-list.html', context={'context': Setup.objects.filter(group__enabled=True),})
...
def group(request, id=None, action=None):
    ...
    if id and action in ['enable', 'disable'] and len(context) > 0 and request.method == "POST":
        group = context[0]
        Group.objects.all().update(enabled=False)
        ...
        group = context[0]
        if request.POST.get('category', None) is None and request.POST.get('link', None):
            group.link = request.POST['link']
        else:
...
def callback(update):
    ...
    log.warning('****************1*******************')
    log.warning(update)
    news_feed_check(is_test=True)
    return HttpResponse(status=200)


def redirector(request, member_id, nid):
    member = get_object_or_404(Member, member_id=member_id)
    news = get_object_or_404(News, id=nid)

    result, created = Result.objects.get_or_create(news=news, member=member, type=member.type)
    if created:
        if member.type == Member.EDITOR:
