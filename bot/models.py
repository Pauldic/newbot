...
class EngagementRule(models.Model):
    ...
    type = models.CharField(primary_key=True, max_length=16, choices=MEDIA_GROUP)

    is_allowed = models.BooleanField(default=True)
    keywords = models.CharField(max_length=512, null=True, blank=True, default=None)
    regex = models.CharField(max_length=512, null=True, blank=True, default=None)
...
class SingletonModel(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        status = cache.delete_pattern(name)
        log.warning("Delete {}, Status: {}".format(name, status))

        if isinstance(self, Group):
            log.warning("Delete Setup")
            log.warning(cache.delete_pattern("{}*".format(Setup.__name__.lower())))
        # if self.can_cache:
        #     self.set_cache()

    def delete(self, name=None, *args, **kwargs):
        if name is None:
            name = self.__class__.__name__.lower()
        log.warning("Delete {}".format(name))
        status = cache.delete_pattern(name)
        log.warning("Delete {}, Status: {}".format(name, status))

    def delete_all(self):
        log.warning("Delete {}".format(self.__class__.__name__.lower()))
        status = cache.delete_pattern("{}*".format(self.__class__.__name__.lower()))
        log.warning("Delete {}, Status: {}".format(self.__class__.__name__.lower(), status))
    ...
    def set_cache(self, obj=None, name=None, timeout=360):
        try:
            if obj is None:
                cache.set(self.__class__.__name__.lower() if name is None else name, self, timeout=timeout)
            else:
                cache.set("{}_obj".format(self.__class__.__name__.lower() if name is None else name), obj, timeout=timeout)
        except Exception as err:
            log.error(err)

    @classmethod
    def reload(cls, id=None, timeout=360):
        cache.expire(cls.__name__.lower(), timeout=0)
        cls.load(id=id, timeout=timeout)

    @classmethod
    def load(cls, id=None, timeout=360, reload=False):
        rules = cache.get(cls.__name__.lower() if id is None else "{}_obj".format(cls.__name__.lower()))
        if reload or rules is None:
            if id is None:
                rules = cls.objects.all()
                log.warning("Total Objects: {} ================ Initialising  Rules ============".format(len(rules)))
                if len(rules):
                    rules[0].set_cache(rules, timeout=timeout)
            else:
                rules = cls.objects.filter(pk=id).first()
                rules.set_cache(timeout=timeout)
        return rules
...
class Member(models.Model):
    ...
    TYPE = ((EDITOR, EDITOR), (MEMBER, MEMBER))

    ACTIVE = "Active"; RESTRICTED = "Restricted"; KICKED = "Kicked"; JOINED = "Joined"; LEFT = "Left"
    STATUS = [(ACTIVE, _("Active")), (RESTRICTED, _("Restricted")), (KICKED, _("Kicked")), (JOINED, _("Joined")), (LEFT, _("Left")), ]
    ...
    username = models.CharField(max_length=128, null=True, blank=True, default=None)
    type = models.CharField(max_length=16, choices=TYPE, default=MEMBER)
    ...
    point = models.DecimalField(max_digits=12, decimal_places=6, default=0)
    batch_id = models.UUIDField(null=True, blank=True, default=None)
    subscribed = models.BooleanField(default=False)
    created = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return "{} {} ({})".format(self.first_name, self.last_name or "", self.type)

    @property
    def level(self):
        if self.type == self.MEMBER:
            return "N/A"
        for l in Level.objects.filter(enabled=True).order_by('point'):
            if l.point > self.point:
                return l.label
...
class Group(SingletonModel):
    id = models.CharField(max_length=16, primary_key=True)
    title = models.CharField(max_length=64)
    username = models.CharField(max_length=64, null=True, blank=True, default=None)
    type = models.CharField(max_length=64)
    link = models.URLField(max_length=128, null=True, blank=True, default=None)
    ...
    @classmethod
    def loadDefault(cls, timeout=360, reload=False):
        default = cache.get("{}_default".format(cls.__name__.lower()))
        log.warning("{}_default".format(cls.__name__.lower()))
        # default = cls.objects.filter(enabled=True).first()
        if reload or default is None:
            default = cls.objects.filter(enabled=True).first()
            default.set_cache(name="{}_default".format(cls.__name__.lower()), timeout=timeout)
        return default
...
class News(models.Model):
    """
    Pending (Transit State) = News just arrived
    Cancelled (Final State) = Admin manually Cancels any further processing on this news
    Rejected (Final State) = Editors rejects the news by Voting
    Accepted (Transit State) = Editors Accepted that the news be published though NOT published yet
    Discarded (Final State) = This news was discarded because it does not receive enough voting to make a decision
    Sent (Transit State) = Editors Accepted and message the news has been published
    Scored (Final State) = All processing on this news has been completed and Editors Scored
    Not Scored (Final State) = All processing on this news has been completed and Editors NOT Scored

    """
    PENDING = "Pending"; REJECTED = "Rejected"; ACCEPTED = "Accepted"; DISCARDED = "Discarded"; SENT = "Sent";
    CANCELLED = "Cancelled"; SCORED = "Scored"; NOT_SCORED = "Not Scored";
    STATUS = ((PENDING, "Pending"), (REJECTED, "Rejected"), (ACCEPTED, "Accepted"), (DISCARDED, "Discarded"),
              (SENT, "Sent"), (CANCELLED, "Cancelled"), (SCORED, "Scored"), (NOT_SCORED, "Not Scored"))

    title = models.CharField(max_length=256, )
    author = models.CharField(max_length=256, null=True, blank=True, default=None)
    source = models.ForeignKey("NewsSource", on_delete=models.PROTECT, null=True, blank=True, default=None)
    url = models.URLField(max_length=256, null=True, blank=True, default=None)
    # url = models.URLField(max_length=128, null=True, blank=True, default=None, unique=True)
    status = models.CharField(max_length=16, choices=STATUS, default=PENDING)
    description = models.TextField(null=True, blank=True, default=None)
    sent_time = models.DateTimeField(null=True, blank=True, default=None)
    score_time = models.DateTimeField(null=True, blank=True, default=None)
    src_time = models.DateTimeField(null=True, blank=True, default=None)
    update = models.DateTimeField(auto_now=True)
    created = models.DateTimeField(auto_now_add=True)
...
class NewsSource(models.Model):
    ...
    source = models.URLField(primary_key=True,)
    enabled = models.BooleanField(default=True)
    latest = models.DateTimeField(null=True, blank=True, default=None)
    update = models.DateTimeField(auto_now=True)
    created = models.DateTimeField(auto_now_add=True)
    ...
    class Meta:
        db_table = "bot_news_source"
        ordering = ('-created', )


class Result_Coin(models.Model):
    channel = models.ForeignKey(Channel, on_delete=models.CASCADE)
    client = models.ForeignKey(Client, on_delete=models.CASCADE)
    source = models.CharField(max_length=32)
...
class Result(models.Model):
    ...
    type = models.CharField(max_length=16, choices=Member.TYPE, )
    read = models.BooleanField(default=True)
    accepted = models.NullBooleanField(null=True, blank=True, default=None)
    late = models.BooleanField(default=False)
    created = models.DateTimeField(auto_now_add=True)

    class Meta:
        ordering = ('-news__created', 'accepted')
        unique_together = ('news', 'member')


class Setup(SingletonModel):
    # group = models.ForeignKey(Group, on_delete=models.CASCADE)
    # send_threshold = models.IntegerField(help_text="Min review (%) before news is sent out")
    max_review_time_editor = models.IntegerField(verbose_name="Max review-time (Editor)", help_text="Maximum time in minutes before news is sent out/discarded")
    max_review_time_user = models.IntegerField(verbose_name="Max review-time (User)", help_text="Maximum time in minutes (from the time the News was sent out) before editors are scored")
    participation_threshold_editor = models.IntegerField(verbose_name="Participation threshold (Editor)", help_text="Min % of participating editors before the participants' votes are considered")
    participation_threshold_user = models.IntegerField(verbose_name="Participation threshold (User)", help_text="Min % of participating users before the participants' votes are considered")
    review_threshold_editor = models.IntegerField(verbose_name="Review threshold (Editor)", help_text="Min +ve % review (based on total votes) before news is sent out/trashed")
    review_threshold_user = models.IntegerField(verbose_name="Review threshold (User)", help_text="Min +ve % review (based on total votes) before editor is scored +ve/-ve")
    positive_reward_point = models.IntegerField(verbose_name="Positive Score point", help_text="Value awarded for passed review")
    negative_reward_point = models.IntegerField(verbose_name="Negative Score point", help_text="Value deducted for failed review")
    enabled = models.BooleanField(default=True)
    updated = models.DateTimeField(auto_now=True)
    created = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return "Editor's Review in {} mins".format(self.max_review_time_editor)

    @classmethod
    def loadSetup(cls, timeout=360, reload=False):
        setup = cache.get(cls.__name__.lower())
        # setup = cls.objects.filter().first()
        if reload or setup is None:
            setup = cls.objects.filter(enabled=True).first()
            try:
                setup.set_cache(name=cls.__name__.lower(), timeout=timeout)
