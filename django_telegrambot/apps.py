...
class DjangoTelegramBot(AppConfig):
    ...
    def ready(self):
        logger.warning("--------Ready()::: {}".format(DjangoTelegramBot.ready_run))
        ...
        logger.warning('Django Telegram Bot <{} mode>'.format(modes[self.mode]))
        ...
        for b in bots_list:
            ...
            if not token:
                break
            logger.warning("--------Ready()::: Bot Token: '{}'".format(token))
            allowed_updates = b.get('ALLOWED_UPDATES', None)
            timeout = b.get('TIMEOUT', None)

            logger.warning("--------Ready()::: Bot Updates: '{}'".format(allowed_updates))
            if self.mode == WEBHOOK_MODE:
                logger.warning("--------Ready()::: Webhooking: '{}'".format(WEBHOOK_MODE))
                try:
                    ...
                    logger.warning("--------Ready()::: URL: '{}'".format(hookurl))
                    logger.warning("--------Ready()::: Cert: '{}'".format(certificate))
                    ...
                    logger.warning("--------Ready()::: Setted: '{}'".format(setted))
                    webhook_info = bot.getWebhookInfo()
                    logger.warning("--------Ready()::: Webhook Info: '{}'".format(webhook_info))
                    real_allowed = webhook_info.allowed_updates if webhook_info.allowed_updates else ["ALL"]
                    logger.warning("--------Ready()::: Real Allowed: '{}'".format(real_allowed))
                ...
                logger.warning("--------Ready()::: Pooling: '{}'".format(self.mode))
                try:
                    ...
                    logger.warning("--------Ready()::: Used Tokens: '{}'".format(DjangoTelegramBot.__used_tokens))
                except InvalidToken:
                    logger.error('Invalid Token : {}'.format(token))
                    return
                except TelegramError as er:
        ...
        logger.warning('Telegram Bot <{}> set as default bot'.format(DjangoTelegramBot.bots[0].username))

        def module_exists(module_name, method_name, execute):
            try:
                m = importlib.import_module(module_name)
                if execute and hasattr(m, method_name):
                    logger.warning('Run {}.{}()'.format(module_name,method_name))
                    getattr(m, method_name)()
                else:
                    logger.warning('Run {}'.format(module_name))

            except ImportError as er:
                logger.warning('{} : {}'.format(module_name, repr(er)))
                return False
        ...
        for app_config in apps.get_app_configs():
            if module_has_submodule(app_config.module, TELEGRAM_BOT_MODULE_NAME):
                ...
                logger.warning("--------Ready()::: Exist? {}".format(module_exists(module_name, 'main', True)))

                if module_exists(module_name, 'main', True):
                    logger.warning('-------Loaded {}'.format(module_name))
